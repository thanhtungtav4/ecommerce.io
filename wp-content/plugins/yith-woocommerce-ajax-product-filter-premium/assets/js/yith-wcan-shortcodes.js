  ...
  yith_wcan_filter_labels_createClass(YITH_WCAN_Filter_Labels, [{
    ...
    value: function _initLabel($label) {
      ...
      $label.on('click', function () {
        return _this.disableLabel($label);
      });
    } // get labels
    ...
    value: function disableLabel($label) {
      var properties = $label.data('filters'); // search for preset
...
function yith_wcan_preset_premium_getPrototypeOf(o) { yith_wcan_preset_premium_getPrototypeOf = Object.setPrototypeOf ? Object.getPrototypeOf : function _getPrototypeOf(o) { return o.__proto__ || Object.getPrototypeOf(o); }; return yith_wcan_preset_premium_getPrototypeOf(o); }
...
var YITH_WCAN_Preset_Premium = /*#__PURE__*/function (_YITH_WCAN_Preset) {
  ...
  function YITH_WCAN_Preset_Premium() {
  ...
  yith_wcan_preset_premium_createClass(YITH_WCAN_Preset_Premium, [{
    key: "isHorizontal",
    value: // check if preset has horizontal layout
    ...
    value: function maybeShowClearFilter($filter) {
      ...
      if (!this.isHorizontal() || this.isMobile) {
        ...
        $clearButton.prependTo($filter.find('.filter-content-footer')).on('click', function (ev) {
          ...
          _this.deactivateFilter($filter, false, yith_wcan_shortcodes.instant_filters);

          _this.maybeUpdateFiltersCount($filter);

          _this.maybeHideClearFilter($filter);

          _this.maybeFilter($clearButton);

          _this.maybeCloseHorizontalToggle($filter.find('.filter-title.collapsable'));
        });
      }
    } // init filter
    ...
    value: function _initToggle($toggle, $container, $target) {
      ...
      $toggle.off('click').on('click', function (ev) {
        ...
        if (isHorizontal && !_this2.isMobile && $toggle.hasClass('filter-title')) {
          // first of all, close other toggles.
          _this2.maybeCloseAllHorizontalToggles($target); // now open current one.
    ...
    value: function maybeCloseAllHorizontalToggles($exclude) {
      this.$preset.find('.filter-title.collapsable').next('.filter-content').not($exclude).hide().prev('.filter-title').removeClass('opened').addClass('closed');
    ...
    value: function _addHorizontalToggleFooter($toggle) {
      ...
      $applyFilterButton.on('click', function () {
        _this3.maybeFilter($applyFilterButton);

        _this3.maybeCloseAllHorizontalToggles($toggle);
      });
